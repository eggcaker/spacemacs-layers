(defun dotspacemacs/layers ()
  (setq-default
   ...
   dotspacemacs-additional-packages '(key-chord ox-reveal hackernews noflet)
...
values."
  ...
  (setq-default
   ...
   ;; Approved: M+ 2m, M+ 1mn, Fira Mono, Monoid, Aurulent Sans Mono,  Menlo, Meslo LG S DZ
   ...
   ;; Which-key delay in seconds. The Guide-key is the popup buffer listing
   ;; the commands bound to the current keystrokes. (default 0.4)
...
(defun u/node-eval ()
  ...
  (let ((debug-on-error t) (start 1) (end 1))
    ...
    (setq deactivate-mark nil))) ; deactive the region, regardless
...
layers configuration. You are free to put any user code."
  ...
  (add-hook 'web-mode-hook 'pelm/web-mode-hook)
  ...
  (evil-leader/set-key (kbd "ah") 'hackernews)
    ...
    ;; temp disabled the eslint for react

    (add-hook 'react-mode-hook #'pelm/disable-eslint)
    )


(defun pelm/disable-eslint()
